...
- Generated app startup should support staged or incremental reveal. If the provider only returns a complete `GenerateUiResult`, the renderer may still play that result out in phases until the protocol supports true patch streaming.
...
user asks for any app or topic
  ...
  -> renderer renders or plays structured blocks in staged updates
